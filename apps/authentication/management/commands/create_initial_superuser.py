...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.NOTICE(
                f'- DJANGO_SUPERUSER_EMAIL: {"Set" if superuser_email else "Not set"}'
            )
        )
        self.stdout.write(
            self.style.NOTICE(
                f'- DJANGO_SUPERUSER_PASSWORD: {"Set" if superuser_password else "Not set"}'
            )
        )
        ...
        if not superuser_email or not superuser_password:
            self.stdout.write(
                self.style.ERROR(
                    "Error: DJANGO_SUPERUSER_EMAIL or DJANGO_SUPERUSER_PASSWORD "
                    "environment variables not set.\n"
                    "This command requires both environment variables to be set with "
                    "valid credentials.\n"
                    "Please set these variables in your deployment environment and "
                    "try again.\n"
                    "Note: After successful superuser creation, it is recommended "
                    "to remove these environment variables for security reasons."
                )
            )
            return
        ...
        try:
            if not User.objects.filter(email=superuser_email).exists():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        "Please remove the superuser credentials from environment variables for "
                        "security."
                    )
                )
            else:
            ...
            self.stdout.write(
                self.style.WARNING(f'Superuser with email "{superuser_email}" may already exist.')
            )
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"Unexpected error creating superuser: {e}"))
            self.stdout.write(
                self.style.ERROR("Please check your database connection and settings.")
            )
