...
class UserLoginAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": e.detail, "detail": first_error}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not user.is_verified:
            return Response(
                {
                    "message": "Email not verified. Please verify using the OTP "
                    "sent to your email.",
...
class UserLogoutAPIView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        response.delete_cookie(settings.JWT_AUTH_COOKIE, samesite=settings.JWT_AUTH_SAMESITE)
        response.delete_cookie(
            settings.JWT_AUTH_REFRESH_COOKIE, samesite=settings.JWT_AUTH_SAMESITE
        )
...
class UserPasswordResetAPIView(APIView):
    ...
    def post(self, request, uid, token, format=None):
        ...
        try:
            ...
            user_email = serializer.validated_data.get("email") or getattr(
                serializer.context.get("validated_user"), "email", None
            )
            ...
            return Response(
                {"detail": "Reset failed. Invalid or expired link.", "errors": e.detail},
...
class VerifyOTPAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if was_limited:
            raise Throttled(
                detail="Too many OTP verification attempts from this IP. Try again later."
            )
        ...
        try:
            ...
            if "non_field_errors" in e.detail and isinstance(e.detail["non_field_errors"], list):
                ...
                error_message = (
                    next(iter(e.detail.values()))[0] if e.detail else "Validation failed."
                )
            elif isinstance(e.detail, list):
            ...
            return Response(
                {"detail": error_message, "errors": e.detail}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ResendOTPAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        was_limited = is_ratelimited(
            request=request, group="resend_otp", key="ip", rate="4/m", method="POST", increment=True
        )
        ...
        try:
            ...
            if "email" in e.detail and isinstance(e.detail["email"], list):
                ...
                return Response(
                    {"detail": error_message, "errors": e.detail},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            else:
                return Response(
            ...
            return Response(
                {"detail": "An unexpected error occurred.", "errors": str(e)},
...
def run_flush_expired_tokens(request):
    ...
    if request.method == "POST":
        if request.headers.get("X-Cron-Secret") != CRON_SECRET_KEY:
            return JsonResponse(
                {"status": "Unauthorized", "message": "Invalid or missing secret key."},
                status=status.HTTP_401_UNAUTHORIZED,
            )
        ...
        try:
            ...
            return JsonResponse(
                {"status": "error", "message": f"Error flushing tokens: {str(e)}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

    return JsonResponse(
        {"status": "method_not_allowed", "message": "Only POST requests are allowed."},
