...
class RedisOTPService:
    ...
    def track_failed_attempt(self, user_id: int):
        ...
        cache.set(
            key,
            new_attempts,
            timeout=self.failed_attempts_window * 60
        )
    ...
    def create_otp_for_user(self, user: Any) -> Dict[str, Any]:
        ...
        if self.is_user_locked_out(user.id):
            ...
            raise ValueError(f"User is temporarily locked out. Please wait {lockout_remaining} minutes before trying again.")
    ...
    def get_otp_for_user(self, user_id: int) -> Optional[Dict[str, Any]]:
    ...
    def verify_otp(self, user: Any, otp_code: str) -> Tuple[bool, str]:
        ...
        if self.is_user_locked_out(user.id):
            lockout_remaining = self.get_lockout_remaining_time(user.id)
            return False, f"User is temporarily locked out. Please wait {lockout_remaining} minutes before trying again."
    ...
    def resend_otp(self, user: Any) -> Tuple[Dict[str, Any], str]:
        ...
        if cooldown_remaining > 0:
            raise ValueError(f"Please wait {cooldown_remaining} seconds before requesting another OTP.")
        ...
        if existing_otp and not self.is_otp_expired(existing_otp):
            ...
            if current_resend_count >= self.max_resend_count:
                ...
                raise ValueError(f"Maximum resend limit ({self.max_resend_count}) reached. Please wait {self.lockout_duration} minutes before trying again.")
            ...
            return existing_otp, f"OTP resent successfully. Attempt {existing_otp['resend_count']}/{self.max_resend_count}"
        else:
    ...
    def get_user_otp_status(self, user_id: int) -> Dict[str, Any]:
        ...
        status = {
            ...
            'lockout_remaining_minutes': self.get_lockout_remaining_time(user_id) if self.is_user_locked_out(user_id) else 0,
        ...
        status['can_resend'] = (
            not status['is_locked_out'] and 
            status['resend_cooldown_seconds'] == 0 and 
            status['resend_count'] < self.max_resend_count
        )
